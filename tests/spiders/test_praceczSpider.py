...
from jobot.spiders.PraceCZ import PraceczSpider
...
class TestPraceczSpider(BetamaxTestCase):  # superclass provides self.session

    def test_parse(self):
        spider = PraceczSpider()
        ...
        for item in items:
            with self.subTest(item=item):
                self.assertTrue("www.prace.cz" in item["link"])
